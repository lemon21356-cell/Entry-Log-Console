(() => {
  ...
  // 현재 URL이 작품 만들기(/ws) 경로인지 확인하는 함수
  const checkWorkspace = () => window.location.pathname.startsWith('/ws');
  ...
  const state = {
    ...
    query: '',
    height: readHeight(),
    collapsed: !checkWorkspace(), // 처음에 작품 만들기가 아니면 접어두기
  };
  ...
  ready(() => {
    ...
    buildPanel();
    render();
    watchUrlChanges(); // 주소가 바뀌는지 감시 시작!
  });
  ...
  // 엔트리는 새로고침 없이 화면이 넘어가므로, 주소 변경을 감지하는 함수
  function watchUrlChanges() {
    ...
    setInterval(() => {
      ...
      if (lastPath !== currentPath) {
        ...
        const isWs = checkWorkspace();
        
        // 작품 만들기 화면으로 들어오면 자동으로 펼치고, 다른 곳으로 나가면 닫기
        if (isWs && state.collapsed) {
          setCollapsed(false);
        } else if (!isWs && !state.collapsed) {
          setCollapsed(true);
        }
      }
    }, 500);
  ...
  // 패널을 접거나 펼치는 상태를 통일해서 관리하는 함수
  function setCollapsed(forceCollapse) {
  ...
  function buildPanel() {
    ...
    // 초기 상태 반영
    if (state.collapsed) {
      ui.root.classList.add('is-collapsed');
    }
    ...
    ui.toggle.textContent = state.collapsed ? '▴' : '×';
    ui.toggle.addEventListener('click', () => setCollapsed(!state.collapsed));
