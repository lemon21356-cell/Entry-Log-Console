(() => {
  ...
  window.addEventListener('message', (event) => {
    ...
    if (payload.type === 'entry') {
      addEntry(payload.level, payload.message, payload.objectName, payload.sceneName);
  ...
  function addEntry(level, message, objectName, sceneName) {
    const normalized = String(level || 'log').toLowerCase();
    state.entries.push({
      id: Date.now() + Math.random(),
      level: levels.includes(normalized) ? normalized : 'log',
      message: stringify(message),
      objectName: objectName ? String(objectName) : '',
      sceneName: sceneName ? String(sceneName) : '',
      time: new Date(),
    });
